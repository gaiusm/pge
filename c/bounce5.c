/* automatically created by mc from ../git-pge-frozen/m2/bounce5.mod.  */
...
static void fireBalls (deviceIf_Colour c);
...
static void fireBalls (deviceIf_Colour c)
{
  ...
  m = macroObjects_moveTo (m, Points_initPoint (Fractions_initFract (0, 5, 20), Fractions_initFract (0, 31, 40)));
  m = macroObjects_circle (m, TRUE, Fractions_zero (), c, Fractions_initFract (0, 1, 40));
  m = macroObjects_rootMacro (m);
  popWorld_mass (Fractions_cardinal (1));
  popWorld_velocity (Points_initPoint (Fractions_half (), Fractions_negate (Fractions_half ())));
...
void _M2_bounce5_init (__attribute__((unused)) int argc, __attribute__((unused)) char *argv[])
{
  popWorld_init (FALSE);
  ...
  placeBoundary ();
  twoDsim_gravity (-4.0);
  fireBalls ((deviceIf_Colour) deviceIf_green ());
  twoDsim_simulateFor (30.0);  /* 15.0 ok  */
}

void _M2_bounce5_finish (__attribute__((unused)) int argc, __attribute__((unused)) char *argv[])
{
}
